...
- Adds a new partnership record with the given user, partner, and notification types.
...
- Updates the `notifyTypes` for the existing partnership.
...
**Description:** Generates and appends new reports for a user's partners using immediate reporting when recent failures exist.
...
- For each of the user's partnerships, it aggregates any unreported failures since the last report.
- If failures are present, it generates a report immediately, marks the included failures as reported, updates the partnership's `lastReportedDate`, and appends the report string to the partner's report log.
